...
function handleSubTap(x, y) {
  ...
  if (currentMode === 'select' && selectedObject) {
    const res = subAPI.handleTap(raycaster, selectedObject);
    if (res && res.action === 'showGizmo' && res.center) {
      createGizmoFor('translate', res.center, selectedObject, 1.0);
      showConfirm();
    }
  }
}
...
function setupPointerEvents() {
  ...
  canvas.addEventListener('pointerup', (e) => {
    ...
    if (dist < 15) {
      if (currentMode === 'select' && selectedObject) {
        handleSubTap(e.clientX, e.clientY);
        lastTapTime = 0;
      } else {
        const now = Date.now();
...
function animate() {
  ...
  orbit.update();
  if (currentMode === 'select' && selectedObject && subAPI.updateTriggerTransforms) {
      try { subAPI.updateTriggerTransforms(selectedObject, camera); } catch(e) {}
    }

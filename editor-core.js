...
function applySelectionUI() {
  ...
  if (selectedObject) {
    ...
    if (currentMode === 'select') {
      ...
      if (center) createGizmoFor('translate', center, selectedObject, 0.9);
      else removeGizmo();
    } else {
...
function handleDoubleTap(x, y) {
  ...
  if (currentMode === 'select' && selectedObject) {
    ...
    if (changed) {
      const center = subAPI.getSelectionWorldCenter();
      if (center) {
        createGizmoFor('translate', center, selectedObject, 0.9); // ✅ smaller
...
function handleSubTap(x, y) {
  ...
  if (currentMode === 'select' && selectedObject) {
    ...
    if (changed) {
      const center = subAPI.getSelectionWorldCenter();
      if (center) {
        createGizmoFor('translate', center, selectedObject, 0.9);
        showConfirm();
      } else {
        removeGizmo();
        hideConfirm();
...
function setupPointerEvents() {
  ...
  canvas.addEventListener('pointerup', (e) => {
    ...
    if (dist < 15) {
      // In subcomponent select mode, single tap should pick immediately (no double-tap required)
      if (currentMode === 'select' && selectedObject) {

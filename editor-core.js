...
function getAutoGizmoScale(worldPos, base = 1.0) {
  if (!camera) return base;
  const d = camera.position.distanceTo(worldPos);
  // Keep a consistent on-screen size across zoom levels (tuned for mobile)
  const s = d * 0.08 * base;
  return Math.min(2.5, Math.max(0.6, s));
}
...
function updateGizmoPose() {
  ...
  if (currentMode === 'select') {
    ...
    currentGizmo.scale.setScalar(getAutoGizmoScale(currentGizmo.position, 0.7));
  } else {
...
function applySelectionUI() {
  ...
  if (selectedObject) {
    ...
    if (currentMode === 'select') {
      ...
      if (center) createGizmoFor('translate', center, selectedObject, getAutoGizmoScale(center, 0.7));
      else removeGizmo();
    } else {
...
function handleDoubleTap(x, y) {
  ...
  if (currentMode === 'select' && selectedObject) {
    ...
    if (changed) {
      const center = subAPI.getSelectionWorldCenter();
      if (center) {
        createGizmoFor('translate', center, selectedObject, getAutoGizmoScale(center, 0.7)); // ✅ auto scale

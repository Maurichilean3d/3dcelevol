...
// Make subcomponent picking easier on touch devices (Points/Lines raycast tolerance in world units)
...
// 🔧 Auto scale gizmo to object size
if (gizmoScale === 1.0 && targetObject) {
  const bbox = new THREE.Box3().setFromObject(targetObject);
  const size = bbox.getSize(new THREE.Vector3()).length();
  const autoScale = Math.max(size * 0.25, 1.0);
  gizmoScale = autoScale;
}
...
/* =============================
   SINGLE TAP VERTEX SELECTION
   (toca una sola vez para activar el gizmo en vértices)
============================= */
function handleSingleTap(x, y) {
  if (currentMode !== 'select' || !selectedObject) return;
  ...
  // Intenta seleccionar vértice/edge/face
  const hit = subAPI.togglePick(raycaster, selectedObject);

  if (hit && subAPI.hasSelection()) {
    const center = subAPI.getSelectionWorldCenter();
    if (center) {
      // Nota: sin cuarto parámetro => auto-escalado dinámico
      createGizmoFor('translate', center, selectedObject);

      // Muestra panel de confirmación (OK/Cancel)
      showConfirm();
    }
  } else {
    // No golpeó nada: limpia selección existente
    if (subAPI.hasSelection()) {
      subAPI.clearSelection();
      removeGizmo();
      hideConfirm();
    }
